...
export default function PremiumLock({ levelTitle, compact = false }: PremiumLockProps) {
  if (compact) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: '#F4ECDD', border: '1px solid rgba(var(--ochre-rgb),0.3)' }}>
        <Star size={16} style={{ color: 'var(--ochre)' }} />
        <div className="flex-1 min-w-0">
          <p className="text-xs font-black truncate" style={{ color: 'var(--ochre)' }}>
            {levelTitle ?? 'Premium Level'}
          </p>
          <p className="text-xs" style={{ color: 'rgba(var(--ochre-rgb),0.8)' }}>R49/month to unlock</p>
        </div>
        <Link
          href="/premium"
        ...
        <ul className="space-y-2 mb-5">
          {[
            ...
            <li key={feature} className="flex items-center gap-2 text-sm" style={{ color: '#8A611F' }}>
              <Check size={14} style={{ color: 'var(--ochre)' }} />
              {feature}
            </li>
          ))}
        </ul>
