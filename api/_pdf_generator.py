...
PLUM=HexColor('#2B1E3F');CYAN=HexColor('#6DD3CE');CORAL=HexColor('#FF5E5B')
AMBER=HexColor('#F5C842');INK=HexColor('#2A1E41');MIDGREY=HexColor('#887890')
LINEGR=HexColor('#D2CED8');XLITE=HexColor('#F5F3F8')
GREENBG=HexColor('#EBF8ED');GREENBRD=HexColor('#96D2A0');REDBOLD=HexColor('#FFB0AE')
W,H=A4;ML=20*mm;MR=20*mm;CW=W-ML-MR
HEADER_H=36*mm;FOOTER_H=18*mm;PAGE_TOP=H-HEADER_H-8*mm;CONT_TOP=H-12*mm-8*mm
...
def generate(data):
    ...
    c.setTitle(data.get('title','Curio Study Material'))
    ...
    def hdr_full(is_memo):
        ...
        if is_memo:
            ...
            c.drawString(42*mm,H-27.5*mm,'MARKING GUIDELINES \u00b7 NOT FOR DISTRIBUTION TO LEARNERS')
        c.setFillColor(HexColor('#B4A8C8'));c.setFont('Helvetica',8)
        gs=f"Grade: {grade}    Subject: {subject}"+(f"  \u00b7  {section}" if section else '')
    ...
    def footer():
        c.setStrokeColor(LINEGR);c.setLineWidth(0.5);c.line(ML,12*mm,W-MR,12*mm)
        c.setFillColor(MIDGREY);c.setFont('Helvetica',7)
        c.drawString(ML,8*mm,'curio learning  \u00b7  curiolearning.co.za  \u00b7  Free for every South African student')
        c.drawRightString(W-MR,8*mm,f'\u00a9 Curio Learning {datetime.now().year}')

    def new_page():
        footer();c.showPage()
        hdr_cont(is_memo_page[0]);return CONT_TOP

    # PAGE 1
    hdr_full(False);y=PAGE_TOP
    y-=4*mm
    c.setFillColor(INK);c.setFont('Helvetica',9);c.drawString(ML,y,'Student Name:')
    c.setFillColor(white);c.rect(ML+32*mm,y-2*mm,100*mm,7*mm,fill=1,stroke=0)
    c.setStrokeColor(LINEGR);c.setLineWidth(0.5);c.rect(ML+32*mm,y-2*mm,100*mm,7*mm,fill=0,stroke=1)
    y-=11*mm
    ...
    if source or topic:
        ...
        c.drawString(ML,y,'  \u00b7  '.join(parts));y-=6*mm

    c.setStrokeColor(LINEGR);c.setLineWidth(0.5);c.line(ML,y,W-MR,y);y-=6*mm

    if passage:
        c.setFillColor(INK);c.setFont('Helvetica-Bold',9.5)
        c.drawString(ML,y,'Read the passage and answer the questions that follow.' if is_junior
                     else 'Read the following passage carefully and answer all questions.')
        y-=8*mm
        pfs=11 if is_junior else 10;plh=5.5*mm if is_junior else 5.0*mm
        paras=[p.strip() for p in passage.split('\n') if p.strip()]
        p_lines=[]
        for para in paras: p_lines.extend(wrap(para,'Helvetica',pfs,CW-14*mm))
        box_h=len(p_lines)*plh+10*mm
        if y-box_h<FOOTER_H+10*mm: y=new_page();y-=4*mm
        c.setFillColor(white);c.rect(ML,y-box_h,CW,box_h,fill=1,stroke=0)
        c.setStrokeColor(LINEGR);c.setLineWidth(0.5);c.rect(ML,y-box_h,CW,box_h,fill=0,stroke=1)
        c.setFillColor(CYAN);c.rect(ML,y-box_h,3*mm,box_h,fill=1,stroke=0)
        c.setFillColor(INK);c.setFont('Helvetica',pfs)
        ty=y-6*mm
        for line in p_lines: c.drawString(ML+5*mm,ty,line);ty-=plh
        y=y-box_h-8*mm

    if y<FOOTER_H+20*mm: y=new_page()
    c.setStrokeColor(LINEGR);c.setLineWidth(0.5);c.line(ML,y,W-MR,y);y-=4*mm
    c.setFillColor(PLUM);c.rect(ML,y-9*mm,CW,9*mm,fill=1,stroke=0)
    c.setFillColor(white);c.setFont('Helvetica-Bold',10)
    c.drawString(ML+4*mm,y-5.5*mm,'Questions' if is_junior else f'QUESTION 1  {section.upper()}')
    c.setFillColor(CORAL);c.setFont('Helvetica-Bold',10)
    c.drawRightString(W-MR-2*mm,y-5.5*mm,f'[{total_marks} marks]')
    y-=9*mm+6*mm

    qfs=11 if is_junior else 10.5;qlh=5.5*mm if is_junior else 5.0*mm
    for q in questions:
        raw=sanitise(q.get('text',''))
        raw=re.sub(r'^\d+[.)]\s*','',raw);raw=re.sub(r'\s*\(\d+\)\s*$','',raw).strip()
        marks=q.get('marks',1);num=q.get('num',1)
        q_lines=wrap(raw,'Helvetica',qfs,CW-20*mm)
        ans_lines=0
        if is_junior: ans_lines=1 if marks<=1 else 2 if marks<=2 else 3 if marks<=3 else 4
        needed=len(q_lines)*qlh+ans_lines*8*mm+8*mm
        if y-needed<FOOTER_H+5*mm: y=new_page()
        c.setFillColor(PLUM);c.setFont('Helvetica-Bold',qfs);c.drawString(ML,y,f'{num}.')
        c.setFillColor(INK);c.setFont('Helvetica',qfs)
        for i,line in enumerate(q_lines): c.drawString(ML+9*mm,y-i*qlh,line)
        c.setFillColor(CORAL);c.setFont('Helvetica-Bold',qfs);c.drawRightString(W-MR,y,f'({marks})')
        y-=len(q_lines)*qlh+2*mm
        if is_junior:
            c.setStrokeColor(HexColor('#AAA5B9'));c.setLineWidth(0.4)
            for _ in range(ans_lines): c.line(ML+9*mm,y-2*mm,W-MR,y-2*mm);y-=8*mm
        c.setStrokeColor(XLITE);c.setLineWidth(0.4);c.line(ML,y-1*mm,W-MR,y-1*mm);y-=5*mm
    ...
    c.setFillColor(white);c.rect(W-MR-35*mm,y-2*mm,35*mm,8*mm,fill=1,stroke=0)
    c.setStrokeColor(LINEGR);c.setLineWidth(0.5);c.rect(W-MR-35*mm,y-2*mm,35*mm,8*mm,fill=0,stroke=1)
    c.setFillColor(MIDGREY);c.setFont('Helvetica',8);c.drawRightString(W-MR-2*mm,y+1*mm,f'/ {total_marks}')
    footer()

    # MEMO
    if memo:
        is_memo_page[0]=True
        c.showPage();hdr_full(True);y=PAGE_TOP
        y-=4*mm
        c.setFillColor(INK);c.setFont('Helvetica-Bold',9.5)
        c.drawString(ML,y,'These marking guidelines are for teacher use only.' if is_junior
                     else 'These marking guidelines are prepared for teacher/examiner use only.')
        y-=6*mm;c.setFont('Helvetica',8.5)
        c.drawString(ML,y,'Award marks for correct meaning. Half marks (\u00bd) may be awarded for partial answers.'
                     if is_junior else 'Credit valid alternative responses where meaning is correct.')
        y-=8*mm;c.setStrokeColor(CYAN);c.setLineWidth(0.8);c.line(ML,y,W-MR,y);y-=7*mm
        c.setFillColor(INK);c.setFont('Helvetica-Bold',9)
        c.drawString(ML+4*mm,y,'Q');c.drawString(ML+22*mm,y,'Answer / Marking Points')
        c.drawRightString(W-MR,y,'Marks');y-=4*mm
        c.setStrokeColor(INK);c.setLineWidth(0.5);c.line(ML,y,W-MR,y);y-=4*mm
        ...
        for m in memo:
            ...
            if y-m_h<FOOTER_H+5*mm:
                footer();c.showPage();hdr_cont(True);y=CONT_TOP-5*mm
            c.setFillColor(GREENBG);c.rect(ML,y-m_h,CW,m_h,fill=1,stroke=0)
            c.setStrokeColor(GREENBRD);c.setLineWidth(0.6);c.rect(ML,y-m_h,CW,m_h,fill=0,stroke=1)
            c.setFillColor(PLUM);c.rect(ML,y-m_h,16*mm,m_h,fill=1,stroke=0)
            c.setFillColor(white);c.setFont('Helvetica-Bold',10)
            ...
            for line in m_lines:
                ...
                for pi,part in enumerate(parts):
                    if part:
                        ...
                        c.drawString(tx_x,ty,part);tx_x+=stringWidth(part,'Helvetica',10)
                    if pi<len(parts)-1:
                        c.setFillColor(CYAN);c.setFont('Helvetica-Bold',9)
                        c.drawString(tx_x,ty,tick_lbl);tx_x+=stringWidth(tick_lbl,'Helvetica-Bold',9)
                ty-=5.2*mm
            if marks_val:
