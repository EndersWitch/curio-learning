...
const NEUTRAL = '#c4b8d8'
const CARD_BG = '#231935'
...
function KeyRuleCard({ concept, index }: LearningCardProps) {
  ...
  return (
    <CardShell icon={<BookMarked size={16} />} label="Key Rule" accentColor={accent} index={index}>
      <h3 className="font-black text-base mb-2 leading-tight" style={{ color: '#F7F7FF' }}>{concept.title}</h3>
  ...
  return (
    <CardShell icon={<PenLine size={16} />} label="Example" accentColor={accent} index={index}>
      ...
      {concept.example && (() => {
        ...
        return (
          <div className="mt-3 rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            {exLines.length > 1 ? (
              <ul className="space-y-1">
                {exLines.map((l, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm font-mono" style={{ color: '#c4b8d8' }}>
              ...
              <p className="text-sm font-mono" style={{ color: '#c4b8d8' }}>
...
function CommonMistakeCard({ concept, index }: LearningCardProps) {
  ...
  const accent = flipped ? '#6DD3CE' : '#FF5E5B'
    ...
    >
      ...
      {!flipped ? (
        <>
          ...
          <p className="text-xs mt-3" style={{ color: 'rgba(255,94,91,0.6)' }}>Tap to see the fix →</p>
        </>
      ) : fixText ? (
        <div className="rounded-xl p-3" style={{ background: 'rgba(109,211,206,0.06)', border: '1px solid rgba(109,211,206,0.2)' }}>
          <ContentBlock text={fixText} accentColor={accent} textColor="#c4b8d8" />
        </div>
      ) : (
...
function SpotDifferenceCard({ concept, index }: LearningCardProps) {
  ...
  return (
    <CardShell icon={<Search size={16} />} label="Spot the Difference" accentColor={accent} index={index}>
      <h3 className="font-black text-base mb-4 leading-tight" style={{ color: '#F7F7FF' }}>{concept.title}</h3>

      <div className="grid grid-cols-2 gap-3 mb-2">
        <div className="rounded-t-xl px-3 pt-2.5 pb-1.5 flex items-center gap-1.5" style={{ background: 'rgba(255,94,91,0.08)', border: '1px solid rgba(255,94,91,0.2)', borderBottom: 'none' }}>
          <X size={12} style={{ color: '#FF5E5B' }} />
          <div className="text-xs font-black" style={{ color: '#FF5E5B' }}>{sideALabel}</div>
        </div>
        <div className="rounded-t-xl px-3 pt-2.5 pb-1.5 flex items-center gap-1.5" style={{ background: 'rgba(109,211,206,0.08)', border: '1px solid rgba(109,211,206,0.2)', borderBottom: 'none' }}>
          <Check size={12} style={{ color: '#6DD3CE' }} />
          ...
          return (
            <div key={i} className="grid grid-cols-2 gap-3">
              <div className="rounded-xl px-3 py-2" style={{ background: 'rgba(255,94,91,0.05)', border: '1px solid rgba(255,94,91,0.15)' }}>
                <p className="text-sm font-semibold" style={{ color: '#c4b8d8' }}><RichText text={row.left} /></p>
              </div>
              <div className="rounded-xl px-3 py-2" style={{ background: 'rgba(109,211,206,0.05)', border: '1px solid rgba(109,211,206,0.15)' }}>
                <p className="text-sm font-semibold" style={{ color: '#c4b8d8' }}><RichText text={row.right} /></p>
              </div>
            </div>
          )
        })}
...
function WhatTestedCard({ concept, index }: LearningCardProps) {
  ...
  return (
    <CardShell icon={<ListChecks size={16} />} label="What You'll Be Tested On" accentColor={accent} index={index}>
      <h3 className="font-black text-base mb-3 leading-tight" style={{ color: '#F7F7FF' }}>{concept.title}</h3>
          ...
          >
            <div className="flex-shrink-0 mt-0.5 flex items-center justify-center"
              style={{
                ...
                background: checked[i] ? accent : 'rgba(255,255,255,0.06)',
                border: checked[i] ? 'none' : '1px solid rgba(255,255,255,0.15)',
                transition: 'all 0.2s', color: '#2B1E3F',
              }}>
              {checked[i] && <Check size={11} />}
            </div>
            <span className="text-sm" style={{ color: checked[i] ? accent : '#c4b8d8', transition: 'color 0.2s' }}>
              <RichText text={line} />
            </span>
          </button>
        ))}
    ...
    <CardShell icon={<Eye size={16} />} label="Try It!" accentColor={accent} index={index}>
      ...
      {!revealed ? (
        <button
          ...
          style={{ background: accent, color: '#2B1E3F' }}
        >
          Reveal Answer →
        </button>
      ) : (
        <div className="rounded-xl p-3 animate-in" style={{ background: 'rgba(109,211,206,0.08)', border: '1px solid rgba(109,211,206,0.25)' }}>
          <div className="text-xs font-black mb-1" style={{ color: '#6DD3CE' }}>Answer</div>
          <p className="text-sm" style={{ color: '#c4b8d8' }}><RichText text={concept.example ?? concept.content} /></p>
        </div>
      )}
    </CardShell>
  )
...
function RealWorldCard({ concept, index }: LearningCardProps) {
  ...
  return (
    <CardShell icon={<Globe size={16} />} label="Real World" accentColor={accent} index={index}>
      <h3 className="font-black text-base mb-2 leading-tight" style={{ color: '#F7F7FF' }}>{concept.title}</h3>
      <div className="rounded-xl p-3 mb-1" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
        <ContentBlock text={concept.content} accentColor={accent} />
      </div>
      {concept.example && <ExampleBlock text={concept.example} accentColor={accent} />}
    </CardShell>
...
function MemoryTrickCard({ concept, index }: LearningCardProps) {
  ...
  return (
    <CardShell icon={<Brain size={16} />} label="Memory Trick" accentColor={accent} index={index}>
      <h3 className="font-black text-base mb-2 leading-tight" style={{ color: '#F7F7FF' }}>{concept.title}</h3>
      <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.04)', border: '1px dashed rgba(255,255,255,0.15)' }}>
        <ContentBlock text={concept.content} accentColor={accent} />
      </div>
      {concept.example && (
        <div className="mt-3 text-sm italic text-center" style={{ color: 'rgba(245,200,66,0.7)' }}>
          &quot;{concept.example}&quot;
        </div>
      )}
    </CardShell>
        ...
        {steps.map((step, i) => (
          <div key={i} className="flex items-start gap-3">
            <div className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-black"
              style={{ background: accent, color: '#2B1E3F', minWidth: 28 }}>
              {i + 1}
            </div>
            <div className="flex-1 pt-0.5">
              <p className="text-sm" style={{ color: '#c4b8d8' }}><RichText text={step} /></p>
            </div>
          </div>
        ))}
      </div>
...
function DefinitionCard({ concept, index }: LearningCardProps) {
  return (
    <CardShell icon={<BookOpen size={16} />} label="Definition" accentColor={NEUTRAL} index={index}>
      <h3 className="font-black text-xl mb-1" style={{ color: '#F7F7FF', fontStyle: 'italic' }}>{concept.title}</h3>
